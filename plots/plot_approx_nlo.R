...
out_data %>% mutate(seed = factor(seed), y = -rel_err_nlo) %>% filter(gamma < 50) -> plot_data
...
p <- ggplot(plot_data, aes(x = gamma-shift_gamma, y = -rel_err_nlo, colour = as.numeric(seed), group = seed)) +
  geom_line(alpha = 0.4) + scale_x_log10() + scale_y_log10() +
  theme_bw(base_size = 16) + scale_color_gradient(low = "#1B9E77", high = "#0B525B") +
  theme(legend.position = "none") + xlab(bquote(gamma ~ "-" ~ .(shift_gamma))) + ylab("Error with next-order correction") +
  geom_smooth(data = subset(plot_data, gamma >= 8),
              method = "lm", se = FALSE, linetype = "dotted", linewidth = 0.4) + 
  geom_abline(slope = -2, intercept = brown_intercept, colour ="#8C6D31", linewidth=1.5) +
  geom_abline(slope = -1, intercept = purp_intercept, colour ="darkorchid4", linetype = 3, linewidth=1.5) +
  annotate("text", x = Inf, y = Inf, label = as.expression(bquote(n == .(n))), 
           hjust = 1.5, vjust = 1.5, size = 6)

print(p)

ggsave(paste0("convergeapprox",n,"_nlo.pdf"), p, height = 4, width = 6)
